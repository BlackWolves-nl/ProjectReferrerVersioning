...
/// </summary>
internal class DiffLineProcessor(string fileName)
{
    private readonly string _fileName = fileName;
    
    // Verbatim regex strings: use doubled quotes for literal quotes inside pattern
    private static readonly Regex _nugetRegex = new(
        @"<PackageReference[^>]*Include=""([^""]+)""[^>]*Version=""([^""]*)""[^>]*>",
        RegexOptions.Compiled);
        
    private static readonly Regex _projectRefRegex = new(
    ...
    private static readonly Regex _assemblyInfoRegex = new(
    ...
    private bool ProcessVersionChange(string trimmed, bool isAdd, FileDiffResult result)
    {
        ...
        ProjectVersionChange change = new()
        {
            FileName = _fileName,
            OldVersion = isAdd ? null : match.Groups[2].Value,
            NewVersion = isAdd ? match.Groups[2].Value : null,
